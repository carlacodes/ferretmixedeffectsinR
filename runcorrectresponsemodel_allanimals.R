...
modelreg_reduc1 <- lmer(
  correctresp ~ pitchoftarg+trialNum+pastcorrectresp+pastcatchtrial+(talker+timeToTarget+side+precur_and_targ_same |ferret),
  data=df, REML = FALSE, )#control = lmerControl(optimizer ="Nelder_Mead")

modelreg_reduc2 <- lmer(
  correctresp ~ pitchoftarg+stepval+trialNum+pastcorrectresp+pastcatchtrial+(talker+timeToTarget+side+precur_and_targ_same |ferret),
  data=df, REML = FALSE,)

modelreg_reduc3 <- lmer(
  correctresp ~ pitchoftarg+stepval+talker+trialNum+pastcorrectresp+pastcatchtrial+(talker+timeToTarget+side+precur_and_targ_same |ferret),
  data=df, REML = FALSE,)

modelreg_reduc4 <- lmer(
  correctresp ~ pitchoftarg+stepval+talker+side+trialNum+pastcorrectresp+pastcatchtrial+(talker+timeToTarget+side+precur_and_targ_same |ferret),
  data=df, REML = FALSE, )

modelreg_reduc5 <- lmer(
  correctresp ~ pitchoftarg+stepval+talker+side+timeToTarget+trialNum+pastcorrectresp+pastcatchtrial+(talker+timeToTarget+side+precur_and_targ_same |ferret),
  data=df, REML = FALSE)

modelreg_reduc55 <- lmer(
  correctresp ~ pitchoftarg*stepval+talker*stepval+side+timeToTarget+trialNum+pastcorrectresp+pastcatchtrial+(talker+timeToTarget+side+precur_and_targ_same |ferret),
  data=df, REML = TRUE)
...
modelreg_reduc6 <- lmer(
  correctresp ~ pitchoftarg+stepval+talker+side+timeToTarget+AM+trialNum+pastcorrectresp+pastcatchtrial+(talker+timeToTarget+side+precur_and_targ_same |ferret),
  data=df, REML = FALSE)
# talker*(pitchoftarg)+side + talker*stepval+timeToTarget
modelreg_reduc7 <- lmer(
  correctresp ~ pitchoftarg*stepval+talker*pitchoftarg+side+timeToTarget+trialNum+pastcorrectresp+pastcatchtrial+(talker+timeToTarget+side+precur_and_targ_same |ferret),
  data=df, REML = FALSE)
...
modelreg_reduc9 <- lmer(
  correctresp ~ pitchoftarg*stepval*talker+trialNum+(talker+timeToTarget+side+precur_and_targ_same |ferret),
  data=df, REML = FALSE)
modelreg_reduc10 <- lmer(
  correctresp ~ pitchoftarg+stepval+trialNum+pastcorrectresp+pastcatchtrial+precur_and_targ_same+(talker+timeToTarget+side+precur_and_targ_same |ferret),
  data=df, REML = FALSE)
